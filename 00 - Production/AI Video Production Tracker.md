...
| AIV-041 | Good Instinct | Brendan | Standup: offers to help ("second pair of eyes"); takes Liam's "it's the deep end" condescension gracefully; glad to be invited in. Same locked look/voice as `AIV-012`. | ~10s | 🧠 | 🧠 |
| AIV-042 | Good Instinct | Brendan | Huddle: into the bug with Liam; floats three approaches, concedes the one that's genuinely right (no prod data into the model), then lands the Socratic "what's `getOrCreateSession` keyed on?" question; the modest deflection ("lucky guess") and the flicker of something he can't name. **Cut to leave a gap for Liam's live realization** (he does the on-stage work). | ~30s | 🧠 | 🧠 |
...
| AIV-050 | Best Practices | Kristina (PM) | Agent-first standup: the board moves fast around Liam; asks kindly how he's getting on with the new workflow; notes Marcus shipped three same-shape tickets while Liam's still on one; takes his "I'm doing it properly" without a fight and lands the load-bearing "I just need you on board." Same locked look/voice as `AIV-010`. | ~13s | 🧠 | 🧠 |
| AIV-051 | Best Practices | Brendan | Standup: offers to hop on and help unblock — warm, no agenda (echo of `AIV-041`). Same locked look/voice as `AIV-012`. | ~8s | 🧠 | 🧠 |
| AIV-052 | Best Practices | Brendan | Huddle: honestly impressed by the craft of Liam's rig before he clocks its cost; floats the simpler path three times, conceding Liam's half-right objection each time (proprietary code, blind-accept, ownership); never gloats when his own screen wins; lets it go at the end with the Scene 06 flicker, older now. **Cut to leave gaps for Liam's live lines.** Same locked look/voice as `AIV-012`. | ~30s | 🧠 | 🧠 |
| AIV-053 | Best Practices | Liam's screen — the rig (screen graphic, not a character) | On the shared screen: a terminal where a small **local** model emits tokens slowly; a config of named agents (planner → critic → executor → verifier) looping; an elapsed timer ticking past 40 min; a git diff where nearly every AI-authored line is struck through and hand-rewritten. Later it invents a helper method that isn't in the repo (starved of context) and Liam strikes it out as proof. Screen only, no voice; stage direction. | ~15s | 🧠 | 🧠 |
| AIV-054 | Best Practices | Brendan's screen — the plain session (on-screen UI, not a character) | On Brendan's own laptop (audience only — Liam can't see it): the **hosted** tool given a *scoped* slice (one file + its test, **no prod data, zero retention**) returns the exact retry fix as a short diff with a green test — done while Liam is still explaining his harness. Mirror of `AIV-044`; the contrast Liam never sees. Screen only, no voice. | ~12s | 🧠 | 🧠 |
