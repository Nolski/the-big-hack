...
| AIV-015 | Cold Open — The Standup *(PR-review coda)* | Brendan | The patient, well-reasoned adopter: concedes the precedent risk and that the PR should've been split, reframes review-the-intent-not-the-diff, risk-based reading, human-written integration tests, staged rollout as the safety net; lands "it's not coming for the thinking — it's coming for the typing." Never defensive, never attacks. | ~25s | 🧠 | 🧠 |
| AIV-016 | Cold Open — The Standup *(PR-review coda)* | THE PR — the diff (screen share) | The same merged API developer portal PR from `AIV-013`, now open and scrolled — file tree, the portal's services, the auth/token section Liam keeps circling | ~10s | 🧠 | 🧠 |
| AIV-017 | Cold Open — The Standup *(PR-review coda)* | Plan + transcript (screen capture, not a character) | On screen: the plan-mode plan written before any file was touched — the portal's services, secrets in Vault, the staged rollout, the keys flagged as a tradeoff — then the long session transcript of Brendan's corrections and the tool's revisions. Screen only, no voice. Dims out last as the rewind begins. | ~12s | 🧠 | 🧠 |
| AIV-018 | Cold Open — The Standup *(PR-review coda)* | CI / canary dashboard (screen graphic) | Green integration suite against a partner sandbox; a flagged 5%→100% staged rollout from Thursday; flat error rate / latency. Text-on-screen, no voice — the safety net that replaced the midnight line-read. | static | 🧠 | 🧠 |
...
| AIV-040 | Good Instinct | Kristina (PM) | Day-two standup: pushes Liam for a direction on the still-open signup race condition (the last unpointed item; "people upstairs keep asking"); blesses the pairing with Brendan. Same locked look/voice as `AIV-010`. | ~14s | 🧠 | 🧠 |
...
| AIV-044 | Good Instinct | Brendan's screen — Claude session (on-screen UI, not a character) | On screen (audience only — Liam can't see it): the tool finds the race by static inspection of the pasted handler (**no** prod data) — thinking, then the walk back up to the pre-commit call, the lit line, the one-line fix + the shared-store smell flagged. The machine's first real engineering turn; mirror of the steam looms. Screen only, no voice; written as stage direction. | ~30s | 🧠 | 🧠 |
| AIV-045 | Heads Together | Brendan (the debrief) | Slack 1:1: brings the good news, then honestly can't take the credit ("I just pasted it into Claude"); tries to shield Liam ("don't make it a thing") and is too honest to pull it off; the flicker of guilt, now with words. Same locked look/voice as `AIV-012`. **Can render as one two-hander clip with `AIV-046`** (no live actor to time against). | ~25s | 🧠 | 🧠 |
| AIV-046 | Heads Together | Kristina (the debrief) | Slack 1:1: genuine delight ("you put your heads together!") that curdles the instant she does the math — weeks of the senior engineer vs. an afternoon and a paste; not gloating, faintly unsettled, then the bright manager clicks back with something new tucked behind it ("there's something here"). Same locked look/voice as `AIV-010`. | ~25s | 🧠 | 🧠 |
