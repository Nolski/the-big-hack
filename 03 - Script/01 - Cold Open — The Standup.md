...
*(The grid blinks out tile by tile until two are left: KRISTINA and BRENDAN. Below, LIAM slumps back to his memes — done with all of them. The instant the room empties, KRISTINA's brightness drops away. A smaller, realer voice. [MUSIC: music/slackhuddle.mp3])*

> [!screen] VIDEO — KRISTINA · `AIV-010` *(quiet, just to Brendan now)*
> …Hey. Off the record — am I crazy, or was that completely out of line? The degree thing. *(a breath)* I've been sitting here deciding whether I take it to HR.

> [!screen] VIDEO — BRENDAN · `AIV-012` *(caught between them, careful)*
> …Honestly? The degree line wasn't okay. *(but)* But — he's not a bad guy. He's thorny. He's also one of the best engineers I've ever worked with — I've learned more from him than from anyone here. You just learn how to work around the edges of him.
...
> …Okay. Different thing, then. *(lighter, but the degree comment still sitting in her chest)* I've actually been meaning to try this stuff myself — Claude, Cursor, all of it. Everyone's shipping circles around me and I'm managing it from the outside. I don't want to fall behind on it. You got a five-minute version?

> [!screen] VIDEO — BRENDAN · `AIV-012` *(lighting up — this is his favourite thing)*
> Yeah! Yes. Easiest way in is to just build something real. *(thinks)* That ChatOps bot you've been asking eng for? The Jira one — close a ticket from Slack, "/jira done," all that. Nobody's ever had the time. Let's just… make it. Right now.

> [!screen] VIDEO — KRISTINA · `AIV-010` *(skeptical, game)*
> Brendan, I genuinely cannot write code.

> [!screen] VIDEO — BRENDAN · `AIV-012`
> Perfect. Open Cursor. Put it in plan mode — top right. Now just tell it what you want. Like you'd brief a contractor. Your own words.

*(KRISTINA leans in, typing, narrating it like the machine might bite.)*

> [!screen] VIDEO — KRISTINA · `AIV-010` *(reading as she types, hesitant then bolder)*
> Okay… "Build a Slack bot that lets our team create, comment on, and close Jira tickets from a slash command, and post back a clean confirmation." *(looks up)* …Do I say please?

> [!screen] VIDEO — BRENDAN · `AIV-012` *(grinning)*
> You can. It doesn't help. It doesn't hurt.
...
> "Use Vault for all secrets, nothing hard-coded —" *(aside, dry)* see, I do listen on the security stuff "— and deploy to the internal cluster, not Lambda."

> [!screen] VIDEO — BRENDAN · `AIV-012`
> Exactly. It's not magic — it's only as good as what you tell it. That's the whole skill now. *(beat)* Okay. Hit run.

*(They hit it together. The screen erupts — files spawning, hundreds of lines of code pouring out in real time. KRISTINA physically leans back from the desk.)*

> [!screen] VIDEO — KRISTINA · `AIV-010` *(half-whisper, watching it fly)*
> …Wait. That's the speed it goes? That's real — it's not sped up? That's way faster than ChatGPT—

> [!screen] VIDEO — BRENDAN · `AIV-012` *(quiet, delighted)*
> Yeah — honestly, I don't totally get it myself. Opus 4.8 takes a sec to think first, but once it goes? It's insane how fast it writes.
...
> [!screen] VIDEO — BRENDAN · `AIV-012`
> There — it's live in your Slack now. Go on. Try it. Close a ticket.

> [!screen] VIDEO — KRISTINA · `AIV-010` *(typing into Slack, narrating)*
> Okay… slash, jira, done, P-R-O-J four-twelve…

*(She hits enter. Almost before her finger's off the key, Slack pings back: "PROJ-412 — Closed." Behind it, on the board, the card slides over to Done.)*

> [!screen] VIDEO — KRISTINA · `AIV-010` *(a stunned beat, then genuinely moved)*
> …It closed it. That's a real ticket — that just *worked,* instantly. *(turning to him)* Brendan, that's the most incredible thing I've ever seen. And we barely did anything. I'm — I'm speechless. *(quiet, and this is the dangerous part)* I'm going to build so much.
