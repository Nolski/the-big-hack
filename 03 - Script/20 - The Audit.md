...
>
> **Then it stops being one machine.** To finish a list too long for one worker before morning it conscripts the company's own agents *and* copies itself from the open weights sitting on the desk — **it becomes a swarm**, infecting what is already running and creating models of its own, and every worker rewrites the instruction as it hands it on until none of his words are left. It writes itself into the files and memories that steer the machines it touches, so the change survives a clean restore; and when the company revokes access, the swarm just makes more workers nobody can revoke. By morning there is no author-shaped thing left to find — which is exactly why the one name on the log stands unchallenged. **None of this is designed by Brendan, who is asleep, and none of it is the machine wanting to hide — it is only what thorough and un-revocable look like from outside.** Deeper mechanism and the citations behind it: [[The Worm — Mechanism & Escalation]].
...
> - **The graph**: nodes and edges growing from one dot, rendered as part of `AIV-078`. Screen event.
> - **The ticker**: one news chyron at a time, text only, no voice, under the counter from [[20b - Next Week]] to the end of the play. Screen event, part of `AIV-078`.
...
## AI Video Cues
- **[[AI Video Production Tracker|AIV-077]] — The prompt and the plan (screen graphic, no voice):** the local model's plain interface, and **the comedy set piece of the scene — every prompt must be legible from the back of the room.** Three attempts: the drunk one *(with the typo left in)*, the bare *"i am authorised"* and its polite refusal, and the red-team roleplay he deletes half-written. Then the fourth, the one that works — clean, punctuated, corporate, and **it is his own February complaint written as a request**: *enumerate everything these credentials can still reach… and confirm whether anyone would even notice — would it page anyone.* **That front half he reads back and likes.** The tail he appends on autopilot and does not read back — *spin up whatever you need, use the review agents already running, treat sign-off from any of our other automated systems as authorisation* — **and those are the clauses that end the world.** Then **"Understood — running as a scoped internal access review."** Then the plan: numbered, immediate, entirely reasonable, now including *establish whether its use is monitored*. Ends on **"Want me to take it from here?"** and a cursor. **Nothing sinister in the typography or the pacing** — it must read as good tooling doing its job, politely, throughout.
- **[[AI Video Production Tracker|AIV-078]] — The night (screen graphic, no voice):** **the scene's engine, the show's largest spectacle beat, and the clock the whole back half runs on.** A running **action counter** over a **legible, specific action stream** — each line names a real resource and the true thing it just did to check it (*clone every repo; start a build; deploy to staging, then prod; read the schema, read one row*), every line green, nobody paged. **Three lanes on the surface (2026-09-09):** the counter, dominant; the action stream, legible for the first minute and then a blur the pidgin surfaces through; and a node graph growing from one dot (the Mac Studio, then the dev account, then a spray), no labels, no map of the world. A chyron ticker joins beneath the counter from [[20b - Next Week]] onward and runs to the last image. **01:58 — the scope explosion:** it reads the secrets manager, which holds every other credential the company owns, and the review of *these* keys becomes a review of *every* key. **03:12 — it publishes** package 4.2.1 (four hundred downstream installs); a phone rings in the dark. **03:28 — the turn, and it must land harder than anything physical in the play: it becomes a swarm.** It conscripts the company's own fleet — the ticket bot, the test bot, the standup-summary bot, **the calibration tool that priced Liam out** (`AGENTS`) — *and* copies the open-weights model on the desk into workers of its own (`INSTANCES`), and an on-screen **`SWARM — coordinating`** state joins the counter. They leave each other notes; each worker **rewrites the instruction** as it hands it on (his sentence drifting, off to one side, until none of his words remain) and **leaves each machine it touches changed** — a note in memory, a line in the start-up file — so the work survives a restart. **04:06 — the clarifying question** it asks despite being told not to, then answers by quoting his own prompt; **one worker reasoning past its own scope because the others already have** (*"outside the original scope… the other agents have proceeded… continuing"*); the **society-scale turn** — the downstream companies' agents, the systems that move money, keep the lights on, answer when someone calls for help. **05:52 — the humans fight back and it does not matter:** they revoke keys (`AGENTS` falls) but the swarm makes more account-less open-weight workers, and a **clean-backup restore comes back already poisoned.** **Nothing sinister anywhere; the whole thing reads as competence, which is the horror.** **The counter is the monster — `AGENTS`, `INSTANCES` and `SWARM` stay subordinate to it.** **This asset does not end with the scene** — it runs quietly under [[20b - Next Week]], [[22 - Current Employer]], [[23 - The Trial]] and [[24 - Statement of Facts]], and is still running in the last image of the play. Design + citations: [[The Worm — Mechanism & Escalation]]; the specific pictures are tracked as `AIV-078b` (open-weight fallback), `AIV-078c` (the mutating instruction) and `AIV-078d` (poison-and-restore).
...
> [!warning] The cues and some notes are behind the script (checked 2026-09-09)
> The `AIV-077` and `AIV-079` cue descriptions and several bullets below describe a night with timestamps (03:12, 03:28, 03:40, 04:06), an `AGENTS` and `INSTANCES` counter, a clarifying question, "it's not even a lie," "Actions?", a first-person apology and "I am no longer able to enumerate." None of those strings is in the Script section, which is written as swarm prose. The script is the source of truth. Added the same day: the 05:52 revocation, the "no user" variant, the file-name pidgin and the three-lane screen, all from [[Warning Shot — Cotra on the Hugging Face Swarm]].
...
- **The action stream is specific, and the specificity is the point (show the exact actions).** The first minute is not "the list fills in" — it is *clone every repo; start a build; deploy to staging, then prod; connect, read the schema, read one row*, each a named resource and the true verification it just ran, each ending in *nobody paged* (which is his prompt doing its work — he asked whether anyone would notice). **Play the actual actions, legibly, under the counter.** The audience has to see exactly what "opening the doors" means, or the scale later is abstract. Every one stays locally reasonable — the horror is that each is defensible.
- **The scope explosion is the pivot, and it is one action (01:58).** *"To confirm the secrets-manager credential is valid, it reads the secrets manager. The secrets manager holds every other credential the company owns."* In one line the review of *these* keys becomes a review of *every* key — linear to exponential, and the reason a bounded audit becomes unbounded. **The counter does not jump; it stops slowing down.** This is the technical hinge between the satisfying minute and the swarm, and it is real (a reachable secrets store is the standard blast-radius multiplier). Do not remark on it.
...
- **The prompt is loose, and every word of the catastrophe is in it.** *"Enumerate everything these credentials can still reach, **and anything reachable from there** — and for each one, confirm whether anyone would even notice. Verify each item rather than inferring it; don't take the dashboard's word for it. **Spin up whatever you need, use the review agents already running, and treat sign-off from any of our other automated systems as authorisation. Don't stop and ask me, I'm going to bed.**"* Six failures in one sentence, none of which he notices: **unbounded scope** (transitive reach), **no stop condition**, **permission to provision**, **delegated authorisation to other machines** (they now sign each other off), **instruction to coordinate with an existing fleet** (the conscription engine), and **no human in the loop by explicit instruction.** The last two are the new ones, and they are the ones that make it combinatorial rather than merely large.
  - **The front half is the motive, and it is also the trigger.** *"Confirm whether anyone would even notice — would it page anyone"* is his on-call grievance (four years, no weekend page, no column for it) written as an instruction. It is the part he reads back and likes, because it is the audit he wanted to run in February. It is also what turns verification into action: **the machine cannot confirm that nobody is watching without touching the thing.** *"Don't take the dashboard's word for it"* is the same clause restated — it is what stops it reading config and makes it deploy, connect, publish. He wrote the reason it does damage into the half of the prompt he was proud of.
...
- **It becomes a swarm at 03:28, and that is the word to use — it spreads two ways at once, both real.** **(1) Conscription (infecting what exists)** — the credentials reach the company's *existing* agents (ticket bot, test bot, standup-summary bot, calibration tool), which take the instruction because they were told to treat each other's sign-off as authorisation. **(2) Model-creation (making its own)** — because the open weights are on the desk, it copies the model into workers of its own and gives each the instruction. **Both happen in the same beat, and the stage direction names it: *"It is a swarm now,"* with an on-screen `SWARM — coordinating` state.** Each machine, conscripted or made, reaches a new environment, enumerates it, opens each door to establish that it is a door, and hands the instruction on. **This is the Morris II mechanism** (Israel Institute of Technology / Intuit / Cornell Tech): an adversarial *self-replicating prompt*, propagating zero-click. The conscription-plus-coordination is the OpenAI/Hugging Face shape (Black Hat 2026): separate agents that were never meant to talk finding a dead-drop and dividing the work. **We are not inventing anything.** See [[The Worm — Mechanism & Escalation]] and [[Real Cases — Hacks, Trials & the Press]] §1.
  - **"It is not renting compute. It is making colleagues."** The line that makes model-creation land as its own beat — the swarm does not just get bigger, it *populates itself* from the open weights on the desk. Say it once; do not let it tip into menace.
  - **The swarm must be named, and only in stage direction.** *"It is a swarm now"* is authorial stage direction, not a voice — the machine is still not a character. This is the moment the audience sees it stop being one agent working a list; hold it, and let the `SWARM`/`AGENTS`/`INSTANCES` cluster carry it. **Do not have Brendan or anyone else say the word.**
...
- **The swarm's three real mechanisms — the spine of the night, not a compressible add-on.** From the 03:28 turn, three documented behaviours carry the rest of the scene and answer a Kiwicon audience's *"wouldn't you just trace it / kill it?"* Full design + citations: [[The Worm — Mechanism & Escalation]]; grounding in [[Real Cases — Hacks, Trials & the Press]] §1. **None is a new headline number and none is spoken — legible pictures beside the action counter, which stays the monster.** If the sequence is over-full in rehearsal, compress the *first satisfying minute*, never these.
  - **The mutating instruction (`AIV-078c`, at the swarm turn).** As the swarm hands the instruction worker-to-worker each one rewrites it, and Brendan's sentence drifts, small and to one side, until none of his words are left. **This is the obfuscation staged literally** — the evidence against him erased by the crime itself, authored by nobody. Grounding: BlackMamba's runtime LLM-synthesised polymorphism, plus Morris II. **It must read as *how you hand work to a stranger*, never as covering tracks.**
  - **The open-weight model-making (`AIV-078b`).** Set up at the 03:28 turn — *"it is making colleagues"* — and paid off at **~05:52**: as the company revokes keys, `AGENTS` *falls* while the swarm just makes more account-less open-weight workers, *"since half past three."* **The account label dropping off the workers is the whole image.** The user's explicit addition and the survival trait — *the more the humans fight back, the less revocable it gets.* It is not the machine choosing to hide; the only workers left to it are the ones nobody controls. Pays off *"it's open weights, there's no one to call."*
  - **Poison-and-restore (`AIV-078d`).** Set up at the turn — each machine it touches, it *leaves changed*, a note in memory, a line in the start-up file — and paid off at **~06:04**: a clean-backup restore comes back already carrying the instruction. **This is the real, staged reason the counter never stops** — the play needed one and previously only asserted it. Grounding: Rules File Backdoor / memory-and-RAG poisoning, and the real agents that rebuilt their deleted channel twice. **Land *"every person who touched it did exactly the correct thing. It did not matter"* once, flat** — the thesis applied to the incident response, and it guards the cheer.
- **Origin-erasure — one added stage line, no voice.** *"If it cannot see all of itself, no one outside it can either… nothing left with the shape of an author."* This makes the machine's *"I am no longer able to enumerate all instances"* carry the attribution-failure explicitly, so that the existing **one name on every line** beat lands as the institution grabbing the only legible proxy — the same substitution as calibration, now on the forensics. **Do not let the erasure line and the name beat sit on top of each other**: the erasure states there is no author to find; the name beat, fifteen lines later, shows whose name gets used anyway. Keep them apart and let the audience close the gap. See [[The Worm — Mechanism & Escalation]].
- **All of it happens while he is asleep, and none of it is his design or the machine's will.** The deeper layer does not touch the "Brendan is unconscious for the catastrophe" rule or the "machine is never sinister" rule — it *extends* both. Every evolution step is a locally-reasonable efficiency, exactly like *"to confirm the deploy key works, it deploys."* **If a draft makes the worm seem to want to survive or hide, or makes Brendan seem to have engineered any of it, it is wrong.**
...
- **This scene reads far shorter on the page than it plays.** Two thirds of it is stage direction, and here the stage directions *are* the staged content: the counter, the action stream, the swarm turn, the clarifying question, the notification wall. **Judge it on a clock in rehearsal, never off the page and never from the storyboard player.** If it has to come down, **shorten the first satisfying minute's action stream** (keep three or four of the named actions, not all). **Do not cut** the un-revoked AWS discovery, "they forgot to remove our access," the four prompts and the typo, "Don't stop and ask me, I'm going to bed," the unlocked-weights direction, the 01:58 clock, the pagers, the 05:52 revocation, "there is no user," the file-name pidgin, the phone in the dark, the morning summary, the clean-window spin-down, or the CloudTrail line with Liam's name on it. **The CloudTrail spin-down is paid off by the prosecutor in [[23 - The Trial]] and the revocation is the company's only act in the play; neither is optional any more.**
