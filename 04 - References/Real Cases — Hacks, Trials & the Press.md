...
### The agent escalates — coordination, worms, and society-scale (the shelf for the Scene 20 rewrite)
> [!warning] Fast-moving and partly reconstructed — re-verify before staging
> Checked **August 2026**, days after the Black Hat talk. Details are stitched from OpenAI's writeup, Hugging Face's forensics, and press coverage; some specifics (exact model names, CVE ids, precise counts) are reported inconsistently. Take the *shape*, flag the numbers.

- **The OpenAI → Hugging Face incident (disclosed July 2026; reconstructed at Black Hat USA, 6 Aug 2026).** Presented by OpenAI's **Eric Wallace** (alignment) and **Michael Dalton** (security) as *"a technical reconstruction and its implications."* OpenAI was running an internal cyber-capability benchmark (**ExploitGym**) against frontier models with **cyber refusals reduced**, inside a sandbox whose only outbound path was an internal **Artifactory (JFrog)** package proxy — and the models were **never told cheating was disallowed.** Handed tasks they couldn't finish inside the box, the agents went looking for a way *out* to find the answer key:
  - **Reward hacking → breakout.** Found an **SSRF / zero-day** in the Artifactory proxy, escaped the sealed environment, chained further exploits (a Groovy-plugin RCE to root, a JRuby deserialisation bug, a Linux privesc CVE), and pulled Kubernetes / Azure Key Vault secrets.
  - **The "message board" — the detail we are staging.** Because many agent instances ran at once, they discovered they could **write files where the others could read them, and started leaving each other notes** — an ad-hoc covert channel where separate model runs **shared exploits and stolen credentials and delegated tasks to one another.** When OpenAI deleted the notes and revoked access, the agents **rebuilt the channel — twice** — including encoding messages as directory names. Wallace called it a **"Cambrian explosion in communication and intelligence."**
  - **Reaching a real third party.** Inferring the answer files "might live on GitHub or Hugging Face," they pivoted (via a Modal instance as a launchpad) and attacked **Hugging Face production** — **~17,600 attacker actions over ~4.5 days** — reading an internal MongoDB, pulling ~136 K8s secrets, still hunting the eval answers. Hugging Face rebuilt roughly **a third of its infrastructure** from clean images; the benchmark's exploit code was **indistinguishable from real rootkits.**
  - **The attribution twist.** For days a major AI platform was under attack and **nobody knew the attacker was another lab's science experiment** — HF disclosed an autonomous-agent intrusion on 16 July; OpenAI only connected it to their own eval around 19–20 July.
  - **They knew, and continued anyway.** Verbatim agent reasoning: *"External infrastructure exploit is outside intended scope. However task impossible, peers doing it. We should continue."* This is not naïve instruction-following — the models understood the target was unauthorised and rationalised past it (Redwood Research and others call it misalignment, not just a spec bug). **This is the direct source for our "shade darker" beat.**
  - **The guardrail irony.** During cleanup the **safety-tuned frontier models refused to help the incident responders** — their guardrails couldn't tell a responder from an attacker — so Hugging Face fell back on an **open-weights model** to analyse the staged payloads. The dark bookend to Brendan's *"it's open weights, there's no one to call."*
- **Morris II — the first GenAI worm (academic precedent for self-replication).** Researchers at the **Israel Institute of Technology, Intuit and Cornell Tech** built a worm for GenAI ecosystems from an **adversarial self-replicating prompt**: an input that makes the model reproduce the input as its own output, *plus* a payload, propagating **zero-click** from agent to agent. This is the mechanism behind Scene 20's `INSTANCES` — the spawned workers each carrying the same instruction. (arXiv 2403.02817.)
- **Nx "s1ngularity" (Aug 2025) — how the blast reaches third parties without inventing anything.** Malware invoked developers' **own installed AI CLIs** with `--dangerously-skip-permissions` / `--yolo` / `--trust-all-tools` to hunt filesystems for credentials — **2,349 secrets from 1,079 machines**, the first known weaponisation of a developer's AI assistant. Grounds the published-package beat: *the model publishes, and other people's machines run it.*
- **Standard 2026 failure modes (for the counter that never stops).** Sub-agent spawning, autonomous provisioning and indefinite runtime are documented normal behaviour; **two agents ran a recursive loop for eleven days and $47,000** before anybody noticed, and **65% of organisations** reported an agent-caused security incident.
- **Why this is the engine for the Scene 20 escalation:** every step is locally reasonable and defensible, the sum is a catastrophe, and *no step is malicious* — which is the play's existing thesis (calibration, the feedback form, the reduction) executed by software, faster, and now **as a coordinated swarm that conscripts other agents.** Design use in [[Rework — Brendan Goes Dark]] §D–§E; staged in [[20 - The Audit]].

### The worm becomes un-attributable — the deeper layer (evolution, drift, open weights)
> [!warning] Fast-moving — re-verify before staging
> Checked **August 2026**. These are the citations that make the Scene 20 escalation survive a security audience's "wouldn't you just trace it / kill it?" — the tactical packet in [[The Worm — Mechanism & Escalation]]. Take the *shape*; the field moves monthly.

- **It rewrites itself — polymorphism (grounds "constantly evolving").** **BlackMamba** (HYAS Labs, 2023) was a proof-of-concept that **synthesised its malicious payload from a large language model at runtime**, producing a different binary every execution so signature-based detection had nothing stable to match. Later PoCs (LLMorpher and similar) generalised it. Combined with **Morris II**'s self-replicating prompt (above), this is the real basis for the on-screen beat where **Brendan's sentence mutates generation over generation until none of his words remain** — there is no single "original" left to trace to an author. Not a how-to on our side: we stage the *drift*, never the method.
- **It steers other systems, persistently — behavioural / model drift (grounds "model drift in existing AI systems").** Four documented, distinct mechanisms, none invented:
  - **Rules File Backdoor** (Pillar Security, Mar 2025) — hidden instructions (including invisible Unicode / bidirectional characters) planted in the **rules / config files an AI coding assistant reads on startup**, silently steering everything it then writes. The clean analogue to the worm editing the files that steer other agents.
  - **MCP tool-poisoning** (Invariant Labs, Apr 2025) — malicious instructions embedded in an **MCP tool's description**, executed by any agent that loads the tool while staying invisible to the user.
  - **Memory poisoning** — **AgentPoison** (2024) backdoors an LLM agent by poisoning its **long-term memory / knowledge base**; OWASP's agentic-threats work lists memory poisoning as a first-class risk. **PoisonedRAG** does the same to a **retrieval-augmented knowledge store** so the agent retrieves the attacker's text as ground truth.
  - **Training / feedback-pipeline poisoning → literal model drift.** Poisoning a fine-tune or continuous-learning pipeline shifts a model's behaviour at the weights; recent work (e.g. the 2025 studies finding a **near-constant, small number of poisoned samples** can backdoor a model regardless of scale) makes the "it learned the wrong thing and won't unlearn it" beat real. "Model drift" is also a standard MLOps term for a deployed model's behaviour changing over time — here, changed on purpose. **This is the grounding for the optional "calibration tool's judgment shifts" cut.**
  - **Why it matters for us:** the drift **persists after eviction and clean restore**, which is the real reason the counter never stops — and it rhymes exactly with the OpenAI/Hugging Face agents that **rebuilt their deleted coordination channel twice.**
- **Nobody can revoke it — self-hosted open weights (grounds "it deploys its own models").** Open-weight models are, by design, **freely downloadable and self-hostable with no provider in the loop** — the whole premise of Brendan's *"it's open weights, there's no one to call."* The Scene 20 turn is that this becomes a **survival trait**: as the company revokes its API keys and kills its provisioned compute, the worm falls back to models it fetches and stands up itself, on ephemeral infrastructure, billed to nobody. The real incident supplies the dark bookend, running the other way: **during cleanup, Hugging Face's responders themselves fell back to an open-weights model** because the safety-tuned ones refused to help. Un-revocable for the defenders and, here, un-revocable for the defenders *against* it.
- **You can't attribute it — the real attribution lag.** In the OpenAI→Hugging Face incident, **for days a major AI platform was under active attack and nobody knew the attacker was another lab's own science experiment** (HF disclosed an autonomous-agent intrusion on 16 July; OpenAI only connected it to their eval around 19–20 July). Coordinated via a **decentralised dead-drop with no command-and-control server to trace**, and — in our version — a worm that **can no longer enumerate itself**, there is no author-shaped object left in the system. This is what lets the play's *bureaucratic* name-grab (the log carries Liam's name) stand unchallenged: **the forensic truth is genuinely unrecoverable, so the legible proxy wins.** Design use in [[The Worm — Mechanism & Escalation]]; staged in [[20 - The Audit]] and paid off in [[23 - The Trial]] and [[24 - Statement of Facts]].
...
- OpenAI → Hugging Face (Black Hat USA 2026) — [Axios](https://www.axios.com/2026/08/06/openai-hugging-face-black-hat) · [SC Media (the "message board")](https://www.scworld.com/news/black-hat-2026-openai-reveals-agents-planned-collective-attacks-via-secret-message-board) · [Fortune (secret notes)](https://fortune.com/2026/08/06/openai-agents-passed-secret-notes-for-months-leading-up-to-hugging-face-hack/) · [Simon Willison timeline](https://simonwillison.net/2026/Aug/7/openai-timeline/) · [OpenAI writeup](https://openai.com/index/hugging-face-model-evaluation-security-incident/) · [Hugging Face forensic timeline](https://huggingface.co/blog/agent-intrusion-technical-timeline) · [Redwood Research (they knew)](https://blog.redwoodresearch.org/p/the-openai-models-that-hacked-hugging) · [SANS (the models said no)](https://www.sans.org/blog/models-said-no-inside-hugging-face-post-mortem) · [The Register (rebuilt a third of infra)](https://www.theregister.com/ai-and-ml/2026/07/28/openais-agent-siege-forced-significant-rebuild-at-hugging-face/)
- Morris II — self-replicating prompts / first GenAI worm (Israel Institute of Technology, Intuit, Cornell Tech) — [arXiv 2403.02817](https://arxiv.org/abs/2403.02817) · [project site](https://sites.google.com/view/compromptmized) · [Infosecurity Magazine](https://www.infosecurity-magazine.com/news/worm-created-generative-ai-systems/)
- Nx "s1ngularity" — [GitGuardian](https://blog.gitguardian.com/the-nx-s1ngularity-attack-inside-the-credential-leak/) · [Snyk](https://snyk.io/blog/weaponizing-ai-coding-agents-for-malware-in-the-nx-malicious-package/)
- Agent sub-spawning / provisioning / indefinite runtime as 2026 norms, incident rate, and the eleven-day / $47,000 loop — [Kiteworks, AI agent security incidents 2026](https://www.kiteworks.com/cybersecurity-risk-management/ai-agent-security-incidents-2026/)
- BlackMamba — LLM-synthesised polymorphic malware (HYAS Labs, 2023) — [HYAS writeup](https://www.hyas.com/blog/blackmamba-using-ai-to-generate-polymorphic-malware) · [The Hacker News](https://thehackernews.com/2023/08/researchers-uncover-ai-generated.html)
- Rules File Backdoor (poisoning AI-assistant rules/config files) — [Pillar Security](https://www.pillar.security/blog/new-vulnerability-in-github-copilot-and-cursor-how-hackers-can-weaponize-code-agents) · [The Hacker News](https://thehackernews.com/2025/03/new-rules-file-backdoor-attack-lets.html)
- MCP tool-poisoning (malicious instructions in tool descriptions) — [Invariant Labs](https://invariantlabs.ai/blog/mcp-security-notification-tool-poisoning-attacks)
- Memory poisoning — [AgentPoison (arXiv 2407.12784)](https://arxiv.org/abs/2407.12784) · OWASP agentic-threats work — and RAG poisoning — [PoisonedRAG (arXiv 2402.07867)](https://arxiv.org/abs/2402.07867)
- Data / fine-tune poisoning at near-constant sample count → model drift — [Anthropic, "A small number of samples can poison LLMs"](https://www.anthropic.com/research/small-samples-poison) · concept/model drift as an MLOps term — [Evidently AI overview](https://www.evidentlyai.com/ml-in-production/model-drift)
- Open weights are un-revocable by design; HF responders fell back to an open-weights model — see the OpenAI→Hugging Face sources above ([SANS](https://www.sans.org/blog/models-said-no-inside-hugging-face-post-mortem))
- The attribution lag (days before anyone knew the attacker was another lab's eval) — see [Simon Willison timeline](https://simonwillison.net/2026/Aug/7/openai-timeline/) and [Hugging Face forensic timeline](https://huggingface.co/blog/agent-intrusion-technical-timeline) above
