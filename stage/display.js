...
function freezeClock(){if(!state?.running)return;const dt=Math.max(0,(Date.now()-(state.sentAt||Date.now()))/1000)*(state.rate||1);state.elapsed+=dt;if(state.screenElapsed)for(const side of ['left','right'])state.screenElapsed[side]+=dt;if(state.cue.isSpeaking)state.audioTime=Math.min(state.audioDuration||Infinity,(state.audioTime||0)+dt);state.running=false;state.sentAt=Date.now()}
let lastDraw=0;
function animate(now){
 requestAnimationFrame(animate);
 if(now-lastDraw<32||!state||state.override?.[side])return;
 lastDraw=now;
 const dt=state.running&&!state.blackout?Math.max(0,(Date.now()-(state.sentAt||Date.now()))/1000)*(state.rate||1):0;
