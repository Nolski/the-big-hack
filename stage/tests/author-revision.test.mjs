...
for(const id of ['s01_l4','s01_l8','s01_l39','s06_l34','s06_l42','s06_l51','s07_l25','s11b_l70','s11c_l33','s14_l81','s14a_l42','s19_l75','s20_l32','s20_l43'])assert.equal(by[id].speaker,'narrator',id);
...
const withoutIndex=c=>({text:c.text,speaker:c.speaker,kind:c.kind,state:c.state});
for(const c of baseline.cues)if(['s20b','s22','s23','s23b','s24'].includes(c.scene)&&!['s24_l35','s24_l37'].includes(c.id))assert.deepEqual(withoutIndex(by[c.id]),withoutIndex(c),`${c.id} outside revision scope`);
