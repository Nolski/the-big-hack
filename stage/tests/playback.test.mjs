...
assert.deepEqual(new Set(castFor(source).map(c=>c.id)),new Set(source.cues.filter(c=>c.speaker&&c.kind!=='stage').map(c=>c.speaker)));
...
console.log('Playback checks passed: current cast choices, all cue coverage, live muting, speed limits, stale-audio protection.');
...
// JSON writes absent optional profiles as null. Both mean the default voice.
const noProfile={...c};delete noProfile.voiceProfile;
const profiledSource={...source,audioManifestRequired:true,cues:[noProfile],performanceStarts:[0]};
const record={speaker:c.speaker,text:c.text,voiceProfile:null,audio:'default.wav'};
assert.equal(playbackShow(profiledSource,generated,{[c.id]:record}).cues[0].audio,'default.wav');
assert.equal(playbackShow({...profiledSource,cues:[{...c,voiceProfile:null}]},generated,{[c.id]:{...record,voiceProfile:undefined}}).cues[0].audio,'default.wav');
